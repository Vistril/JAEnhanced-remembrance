...
void ST_ClearTimers(gentity_t* ent)
{
	...
	TIMER_Set(ent, "blasterAltBurst", 0);
	TIMER_Set(ent, "blasterAltCooldown", 0);
	TIMER_Set(ent, "blasterAltShotPace", 0);
	TIMER_Set(ent, "combatStrafe", 0);//WOG: proactive strafing cooldown
	TIMER_Set(ent, "duckToggle", 0);//WOG: duck/stand cycling cooldown
}
...
static void ST_CheckMoveState(void)
{
	...
	if ((NPCInfo->goalEntity != NPC->enemy) && (NPCInfo->goalEntity != NULL))
	{
		...
		{//either hit our navgoal or our navgoal was not a crucial (scripted) one (maybe a combat point) and we're scouting and found our enemy
			...
			{
				int duckTime = (NPC->max_health - NPC->health) * 100;
				TIMER_Set(NPC, "duck", (duckTime > 3000) ? 3000 : duckTime);//WOG: capped at 3s, was uncapped (could be 8s+ for low-health NPCs)
			}
			...
			TIMER_Set(NPC, "roamTime", Q_irand(3000, 6000));//WOG: was 8000-15000, reduced so ranged NPCs reposition more aggressively
			if (Q_irand(0, 3) == 0)
			{
				TIMER_Set(NPC, "duck", Q_irand(1500, 3000));		//WOG: was 5000-10000, shortened for faster duck/stand cycling
			}
			return;
		}
...
void ST_Commander(void)
{
	...
	for (i = curMemberNum; i < lastMemberNum; i++)
	{
		...
		if (NPC->client->NPC_class != CLASS_ASSASSIN_DROID)
		{
			...
			{
				TIMER_Set(NPC, "checkEnemyTooCloseDebouncer", Q_irand(300, 1000));//WOG: was 1000-6000, check very frequently

				float distThreshold = 262144/*512*512*/;//WOG: was 16384 (128*128), ranged NPCs now uncomfortable at 512 units
				switch (NPC->s.weapon)
				{
				case WP_ROCKET_LAUNCHER:
				case WP_FLECHETTE:
...
void Noghri_StickTrace(gentity_t* self)
{
	if (!self->ghoul2.size()
...
void NPC_BSST_Attack(void)
{
	...
	//WOG: Proactive combat strafing — shuffle side to side while holding a combat position
	if ((NPCInfo->squadState == SQUAD_STAND_AND_SHOOT || NPCInfo->squadState == SQUAD_COVER) && enemyLOS
		&& TIMER_Done(NPC, "strafeLeft") && TIMER_Done(NPC, "strafeRight")
		&& TIMER_Done(NPC, "duck")
		&& TIMER_Done(NPC, "combatStrafe"))
	{
		int strafeTime = Q_irand(300, 600);//short shuffle, covers ~3-4 feet lateral
		if (Q_irand(0, 1))
		{
		...
		TIMER_Set(NPC, "combatStrafe", strafeTime + Q_irand(200, 600));
	}

	//WOG: Duck cycling — alternate between crouching and standing while holding position
	if ((NPCInfo->squadState == SQUAD_STAND_AND_SHOOT || NPCInfo->squadState == SQUAD_COVER) && enemyLOS
		&& TIMER_Done(NPC, "duck") && TIMER_Done(NPC, "stand")
		&& TIMER_Done(NPC, "strafeLeft") && TIMER_Done(NPC, "strafeRight")
		&& TIMER_Done(NPC, "duckToggle"))
	{
		if (Q_irand(0, 2) == 0)
		{//~33% chance to duck each cycle
			TIMER_Set(NPC, "duck", Q_irand(1500, 3000));
		}
		TIMER_Set(NPC, "duckToggle", Q_irand(2000, 4000));
	}
	...
	if ( //!TIMER_Done( NPC, "flee" ) ||
		(doMove && !TIMER_Done(NPC, "runBackwardsDebounce") && !enemyLOS))//WOG: only stop facing enemy if we've lost sight — repositioning NPCs keep shooting
	{//running away and can't see enemy
		faceEnemy = qfalse;
	}
	...
	{//try to shoot if it's time
		...
		if (TIMER_Done(NPC, "attackDelay"))
		{
			...
			else if (NPC->s.weapon == WP_BLASTER)
			{
				if (!TIMER_Done(NPC, "blasterAltBurst"))
				{//mid-burst: pace alt-fire slightly below max fire rate
					ucmd.buttons &= ~BUTTON_ATTACK;
					if (TIMER_Done(NPC, "blasterAltShotPace"))
					{
						ucmd.buttons |= BUTTON_ALT_ATTACK;
						TIMER_Set(NPC, "blasterAltShotPace", 75);
					}
				}
				else if (TIMER_Done(NPC, "blasterAltCooldown") && (ucmd.buttons & BUTTON_ATTACK))
				{//not in burst and cooldown is done: start a new burst
					ucmd.buttons &= ~BUTTON_ATTACK;
					ucmd.buttons |= BUTTON_ALT_ATTACK;
					int burstTime = Q_irand(250, 500);
					TIMER_Set(NPC, "blasterAltBurst", burstTime);
					TIMER_Set(NPC, "blasterAltCooldown", burstTime + Q_irand(500, 1500));
				}
				//else: burst ended but still cooling down — let primary fire through as single shots
			}
			else if (NPC->s.weapon == WP_REPEATER
				&& !(NPCInfo->scriptFlags & SCF_ALT_FIRE))
			{//repeater user not already configured for alt-fire
				if ((ucmd.buttons & BUTTON_ATTACK)
					&& enemyDist > (192 * 192) //safe from self-splash
					&& enemyDist < (384 * 384) //within orb effective range (gravity arc)
					&& !Q_irand(0, 2))
				{//close enough: lob an energy orb
					ucmd.buttons &= ~BUTTON_ATTACK;
					ucmd.buttons |= BUTTON_ALT_ATTACK;
					TIMER_Set(NPC, "attackDelay", Q_irand(1000, 2500));
				}
			}
		}
	}
